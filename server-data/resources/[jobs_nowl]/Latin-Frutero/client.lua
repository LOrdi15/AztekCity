        ...
        if PlayerData.job ~= nil and PlayerData.job.grade_name == 'fruitpicker' then
            if not onDuty then
                if (GetDistanceBetweenCoords(pos, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 10) then
                    ...
                    if (GetDistanceBetweenCoords(pos, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        sleep = 6
                        DrawText3D(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z + 0.4, '~g~[E]~s~ - Ropa De Trabajo')
                        if IsControlJustPressed(0, Keys["E"]) then
                            if not InVehicle then
                                exports.rprogress:Custom({
                                    Duration = 2500,
                                    Label = "Colocando Ropa De Trabajo...",
                                    Animation = {
                                ...
                                exports['okokNotify']:Alert("Frutero","Iniciaste el trabajo de frutero.", 2000, 'info')
                             --   Citizen.Wait(2000) --wait para noti
                                exports['okokNotify']:Alert("Frutero","Ve al garage y coge el carro.", 5000, 'error')
                                onDuty = true
                                blips()
                                end)
                            else
            ...
            elseif onDuty then
                if (GetDistanceBetweenCoords(pos, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 10) then
                    ...
                    if (GetDistanceBetweenCoords(pos, Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z, true) < 1.5) then
                        sleep = 6
                        DrawText3D(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z + 0.4, '~r~[E]~s~ - Ropa De Trabajo')
                        if IsControlJustPressed(0, Keys["E"]) then
                            if not InVehicle then
                                exports.rprogress:Custom({
                                    Duration = 2500,
                                    Label = "Colocando Ropa De Civil...",
                                    Animation = {
                                ...
                                exports['okokNotify']:Alert("Frutero","Terminaste tu trabajo", 3500, 'info')
                                onDuty = false
                                working = false
                                hasWashed = false
                                hasBox = false
                    ...
                    if (GetDistanceBetweenCoords(pos, Config.Locations["cardeposit"].coords.x, Config.Locations["cardeposit"].coords.y, Config.Locations["cardeposit"].coords.z, true) < 2.5) then
                        ...
                        if InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                if hasCar then
                                    DepositBack()
                                    exports['okokNotify']:Alert("Frutero","Te regresaron tus 250 por devolver el carro", 3500, 'error')
                                    working = false
                                    hasCar = false
                                    Plate = nil
                                    TriggerServerEvent('angelo-stompy-regresar', source)
                                    for i, v in ipairs(Config.OrangeSpots) do
                                        RemoveBlip(v.blip)
                                        v.taked = true
                                    end
                        ...
                        elseif not InVehicle then
                            ...
                            if IsControlJustReleased(0, Keys["E"]) then
                                ESX.TriggerServerCallback('angelo-stompy-revisar', function(hasMoney)
                                if hasMoney then
                                    ...
                                    exports['okokNotify']:Alert("Frutero","Pagaste 250 por rentar un carro", 3500, 'error')
                            ...
                            if not InVehicle then
                                if hasBox then
                                    if hasWashed then
                                        ...
                                        TriggerServerEvent('angelo-stompy-alexito', AmountPayout)
