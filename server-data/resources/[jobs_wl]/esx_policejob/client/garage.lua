...
VehicleMenu = function(type)
	...
	function(data, menu)
		...
		--TriggerServerEvent('gcm_llaves:addKeys', plate)
		...
		for i = 1, #v.Pos, 1 do
			if IsPedInAnyVehicle(ped, true) then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(ped), v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) <= Config.Distance then
					...
					exports['progressBar']:drawBar((Config.RepairTime * 1000), Config.Progress1)
		...
		for i = 1, #v.Pos, 1 do
			if IsPedInAnyVehicle(ped, true) then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(ped), v.Pos[i].x, v.Pos[i].y, v.Pos[i].z, true) <= Config.Distance then
					...
					FreezeEntityPosition(veh, true)
					exports['progressBar']:drawBar((Config.CleanTime * 1000), Config.Progress2)
...
RegisterNetEvent('balanz-extramenu:openMenu')
AddEventHandler('balanz-extramenu:openMenu', function()
    OpenMainMenu()
end)
