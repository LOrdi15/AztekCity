...
function radarSetSpeed(defaultText)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if gettxt ~= nil then
            ...
            AddTextComponentString("~r~Ingrese un número correcto !")
...
function POLICE_radar()
    ...
    if isRadarPlaced then 
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) < 0.9 then 
            ...
            ClearPrints()
            SetTextEntry_2("STRING")
            AddTextComponentString("~r~No estas al lado de tu radar !")
            DrawSubtitleTimed(3000, 1)
    ...
    while true do
        ...
        if isRadarPlaced then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) > 300 then 
                ...
                ClearPrints()
                SetTextEntry_2("STRING")
                AddTextComponentString("~r~Te has alejado demasiado de tu radar !")
                DrawSubtitleTimed(3000, 1)
        ...
        if isRadarPlaced then
            ...
            if veh ~= nil then
                ...
                if vehSpeedKm < maxSpeed then
                    info = string.format("   Vehiculo ~r~%s ~w~Matricula ~r~%s ~w~Km/h ~g~%s", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                else
                    info = string.format("   Vehiculo ~r~%s ~w~Matricula ~r~%s ~w~Km/h ~r~%s", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                    ...
                    LastInfo = string.format("   Vehiculo ~r~%s ~w~Matricula ~r~%s ~w~Km/h ~r~%s", LastVehDesc, LastPlate, math.ceil(LastSpeed))
                end
