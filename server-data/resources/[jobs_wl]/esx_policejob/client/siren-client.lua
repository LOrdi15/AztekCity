...
CONTROLS	
	...
	Left indicator:		<-	
	Hazard lights:	\/	
	Toggle emergency lights:	Left CTRL	
	Airhorn:	E	(Horn)
	Toggle siren:	Left ALT	
	...
	while true do
			...
			if IsPedInAnyVehicle(playerped, false) then	
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					DisableControlAction(0, 189, true) -- INPUT_VEH_PREV_RADIO_TRACK  
					DisableControlAction(0, 190, true) -- INPUT_VEH_NEXT_RADIO_TRACK 
					...
					if GetVehicleClass(veh) == 18 then
						...
						DisableControlAction(0, 3, true) -- INPUT_VEH_NEXT_RADIO
						DisableControlAction(0, 3, true) -- INPUT_VEH_PREV_RADIO
						DisableControlAction(0, 3, true) -- INPUT_CHARACTER_WHEEL 
						DisableControlAction(0, 80, true) -- INPUT_VEH_CIN_CAM 
					
						SetVehRadioStation(veh, "ON")
						...
						if not IsPauseMenuActive() then
							...
							-- TOG DFLT SRN LIGHTS
							if IsDisabledControlJustReleased(0, 62) and GetLastInputMethod( 0 ) or IsDisabledControlJustReleased(0, 60) and GetLastInputMethod( 0 ) then
							...
							-- TOG LX SIREN
							elseif IsDisabledControlJustReleased(0, 19) and GetLastInputMethod( 0 ) or IsDisabledControlJustReleased(0, 19) and GetLastInputMethod( 0 ) then
							...
							-- POWERCALL
							elseif IsDisabledControlJustReleased(0, 172) and GetLastInputMethod( 0 ) then
								if state_pwrcall[veh] == true then
									PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
									TogPowercallStateForVeh(veh, false)
									count_bcast_timer = delay_bcast_timer
							...
							if state_lxsiren[veh] > 0 then
								if IsDisabledControlJustReleased(0, 80) and GetLastInputMethod( 0 ) or IsDisabledControlJustReleased(0, 80) and GetLastInputMethod( 0 ) then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 1
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1) -- on
							...
							if state_lxsiren[veh] < 1 then
								if IsDisabledControlPressed(0, 80) or IsDisabledControlPressed(0, 80) then
									actv_manu = true
								else
									actv_manu = false
								end
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if not IsPauseMenuActive() then
							...
							-- IND L
							if IsDisabledControlJustReleased(0, 189) and GetLastInputMethod( 0 ) then -- INPUT_VEH_PREV_RADIO_TRACK
								...
								count_bcast_timer = delay_bcast_timer			
							-- IND R
							elseif IsDisabledControlJustReleased(0, 190) and GetLastInputMethod( 0 ) then -- INPUT_VEH_NEXT_RADIO_TRACK
								local cstate = state_indic[veh]
