...
TriggerEvent('es:addGroupCommand', 'comserv', 'admin', function(source, args, user)
	if args[1] and GetPlayerName(args[1]) ~= nil and tonumber(args[2]) then
		TriggerEvent('comiendo_mierda', tonumber(args[1]), tonumber(args[2]))
	else
...
ESX.RegisterCommand({'endcomserv', 'endcomserv'}, 'admin', function(xPlayer, args, showError)
	if args[1] then
	...
	TriggerClientEvent('chat:addMessage', source, { args = { _U('system_msn'), _U('insufficient_permissions') } })
end, false, {help = _U('unjail_people'), params = {{name = "id", help = _U('target_id')}}})
...
RegisterServerEvent('comiendo_mierda')
AddEventHandler('comiendo_mierda', function(target, actions_count)
