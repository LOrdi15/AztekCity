    ...
    while true do
        ...
        if NearBank() and not isBankOpened and NearBank() ~= "update" then
        	if not Config.okokTextUI then
				...
				ESX.ShowHelpNotification( "Presiona ~INPUT_PICKUP~ para acceder a ~b~Banco")
            ...
            if IsControlJustReleased(0, 38) then
				exports["magni-cardswipe"]:cardswipe(
					...
					function()
						exports['okokNotify']:Alert("Banco", "No pudimos leer tu tarjeta, intentalo mas lento", 2500, 'error')
				end)
            end
        ...
        if inRange and not shown then
        	shown = true
        	exports['okokTextUI']:Open('[E] para acceder al banco', 'darkblue', 'left') 
    ...
    while true do
        ...
        if NearATM() and not isBankOpened and NearATM() ~= "update" then
        	if not Config.okokTextUI then
				...
				ESX.ShowHelpNotification( "Presiona ~INPUT_PICKUP~ para acceder al cajero de ~b~Banco")
            	...
            	ESX.TriggerServerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
						...
						TriggerEvent("pNotify:SendNotification",{
							...
							text = "Dirigete a un banco para establecer un código PIN",
							type = "info",
        ...
        if inRange and not shown then
        	shown = true
        	exports['okokTextUI']:Open('[E] para acceder al cajero', 'darkblue', 'left') 
        elseif not inRange and shown then
        	shown = false
        	exports['okokTextUI']:Close()
        end
...
-- RegisterNetEvent("open-atm-angelo")
-- AddEventHandler("open-atm-angelo", function()
-- 	local inRange = false
-- 	local shown = false
		...
		elseif not Config.CustomIBANAllowLetters then
			if tonumber(data.iban) ~= nil then
				...
				TriggerEvent("pNotify:SendNotification",{
					...
					text = "Solo puedes usar numeros en el IBAN",
					type = "error",
