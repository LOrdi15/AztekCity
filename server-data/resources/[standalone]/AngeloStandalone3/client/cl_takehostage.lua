...
local function drawNativeText(str)
 ...
 RegisterCommand("funcionbindnopermitidaonlymenugrande",function()
	if ESX ~= nil then
		PlayerJobInfo = ESX.PlayerData.job

		if PlayerJobInfo ~= nil then
			if PlayerJobInfo.name == "guerrilla" or PlayerJobInfo.name == 'police' then
  	callTakeHostage()
			else 
				exports['okokNotify']:Alert("Criminal", "Solos los grupos criminales de alto rango pueden usar este comando/funcion", 3500, 'error')
			end
			end
			end
 end)
...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas una pistola con munición para tomar un rehen a punta de pistola!")
	end
...
-- RegisterNetEvent("th-menu-f6-criminal") 
-- AddEventHandler("th-menu-f6-criminal", function()
-- 	callTakeHostage()
-- end)
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Presiona [G] para soltarlo, [H] para matarlo")
